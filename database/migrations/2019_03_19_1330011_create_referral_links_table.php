...
class CreateReferralLinksTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('referral_links', function (Blueprint $table) {
            ...
            $table->unsignedInteger('referral_househelp_id');
            $table->string('code', 36)->index();
            $table->unique(['referral_househelp_id', 'user_id']);
            $table->timestamps();

            $table->foreign('user_id')->references('id')->on('users')->onDelete('cascade');
            $table->foreign('referral_househelp_id')->references('id')->on('referral_househelps')->onDelete('cascade');
