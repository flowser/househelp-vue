...
public function index()
    {
        if (auth()->check()) {
            if(auth()->user()->hasRole('Client')) {
                $user =auth('api')->user();
                $client = OrganisationClient::with('user','gender', 'country', 'county', 'constituency', 'ward','education')
    ...
    public function ClientList()
 {
    ...
    public function store(Request $request)
    {
        $this->validate($request,[
            ...
            if (auth()->check()) {
                if (auth()->user()->hasAnyRole(['Superadmin','Client'])) {
                    $organisation = Organisation::first();
                    if ($organisation){
                        $user = new User();
                        ...
                        if($user){
                            $organisation->organisationclients()->save($user, [
    ...
    public function update(Request $request, $id)
    {
       ...
       $user ->givePermissionTo('View Frontend');
           ...
           $currentPassport = $user->organisationclients()->first()->pivot->photo;
           ...
           if($request->photo != $currentPassport){
               ...
               $photo = $user->organisationclients()->first()->pivot->photo;
           }
          //front side id
          $currentFrontside_id = $user->organisationclients()->first()->pivot->id_photo_front;
           ...
           if($request->id_photo_front != $currentFrontside_id){
               ...
               $id_photo_front = $user->organisationclients()->first()->pivot->id_photo_front;
           }

           //backside id
           $currentBackside_id = $user->organisationclients()->first()->pivot->id_photo_back;
           ...
           if($request->id_photo_back != $currentBackside_id){
               ...
               $id_photo_back = $user->organisationclients()->first()->pivot->id_photo_back;
           }
       ...
       if($user){
           ...
           DB::table('organisation_client')->where('user_id', $user->id)
           ->update([
               'gender_id'        => $request->gender_id,
               'active'           => true,
               'id_no'            => $id_no,
    ...
    public function destroy($id)
    {
        ...
        if($user){
            $organisation = $user->organisationclients()->first();
             $currentPassport = $user->organisationclients()->first()->pivot->photo;
                 $ps_Path = public_path()."/assets/organisation/img/clients/passports/";
                 $S_currentPassport = $ps_Path. $currentPassport;
                     if(file_exists($S_currentPassport)){
        ...
        $organisation->organisationclients()->detach($id);
        $user->delete();
    }
}
